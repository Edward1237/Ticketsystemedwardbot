...
async def on_command_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        ...
        pass # Already handled by checks like is_staff()
...
async def create_ticket_channel(interaction: discord.Interaction, ticket_type_name: str, settings: dict):
    ...
    overwrites = {
        ...
        guild.me: discord.PermissionOverwrite(read_messages=True, send_messages=True, manage_channels=True), # Added manage_channels for delete on timeout
...
class AppealReasonModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not self.original_message.embeds:
             await interaction.followup.send(embed=create_embed("Error", "Could not find original appeal embed to update.", discord.Color.red()))
             return
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception):
        ...
        await interaction.followup.send("An unexpected error occurred processing the appeal.", ephemeral=True)
...
class AppealReviewView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not interaction.message.embeds:
            await send_embed_response(interaction, "Error", "Could not find the appeal embed.", discord.Color.red())
            return
        embed = interaction.message.embeds[0]
        if not embed.footer or "User ID:" not in embed.footer.text:
            await send_embed_response(interaction, "Error", "Could not find the User ID in the appeal message footer.", discord.Color.red())
            return

        try:
            user_id = int(embed.footer.text.split(": ")[1])
        except (IndexError, ValueError):
             await send_embed_response(interaction, "Error", "Could not parse User ID from appeal message footer.", discord.Color.red())
             return
...
class ConfirmAppealView(discord.ui.View):
    ...
    async def submit(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            print(f"Error submitting appeal to channel: {e}")
            await interaction.followup.send(embed=create_embed("Error", "An unexpected error occurred while submitting the appeal.", discord.Color.red()))
    ...
    async def on_timeout(self):
        ...
        try:
            ...
            print(f"Failed to edit message on appeal timeout: {e}")
            pass
        await self.cleanup() # Pass interaction=None
...
class AppealStartView(discord.ui.View):
    ...
    async def start_appeal(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if answer3_msg.attachments:
            ...
            if proof_content != "N/A":
                proof_content = f"{proof_content}\n{proof_urls}"
            else:
                 proof_content = proof_urls
    ...
    async def on_timeout(self):
        ...
        try:
            ...
            print(f"Failed to edit appeal start message on timeout: {e}")
...
class TicketPanelView(discord.ui.View):
    ...
    async def send_appeal_dm(self, user: discord.Member, guild: discord.Guild, reason: str):
        ...
        try:
            ...
            print(f"Failed to send appeal DM to {user.id}: {e}")
    ...
    async def tryout_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            timeout_embed = discord.Embed(
                title="Ticket Closed",
                description="This ticket has been automatically closed due to inactivity during the application.",
                color=discord.Color.red()
            )
            try:
                ...
                pass # Channel was already deleted
                 ...
                 print(f"Error: Lacking permissions to delete channel {channel.id} after timeout.")
            except Exception as e:
                print(f"Error during tryout timeout cleanup: {e}")

        except Exception as e:
            print(f"Error during tryout process in {channel.id}: {e}")
            try:
...
class TicketCloseView(discord.ui.View):
    ...
    async def delete_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Defer first to acknowledge, *then* send the confirmation/warning
        await interaction.response.defer()
        embed = discord.Embed(
        ...
        await interaction.followup.send(embed=embed, ephemeral=False) # Make warning visible to others
        ...
        try:
            ...
            pass # Channel already gone
             ...
             # Can't delete, maybe missing perms? Send error message.
             try:
                 await interaction.followup.send(embed=create_embed("Error", "I lack permissions to delete this channel.", discord.Color.red()), ephemeral=True)
             except Exception:
                 pass # Ignore if followup fails
        except Exception as e:
            print(f"Error during ticket deletion: {e}")
    ...
    async def close_ticket_logic(self, channel: discord.TextChannel, user: discord.Member):
        ...
        try:
            await channel.send(
             ...
             # Handle potential file too large error
            if e.code == 40005: # Request Entity Too Large
                await channel.send(embed=create_embed("Transcript Too Large", "The transcript is too large to upload here. The ticket will still be archived.", discord.Color.orange()))
            else:
        ...
        try:
            # Generate a more unique closed name to avoid conflicts
            ...
            # Attempt to remove buttons from the last message sent by the bot (transcript msg)
            async for msg in channel.history(limit=5):
                if msg.author == self.bot.user and msg.embeds:
                    try:
                        await msg.edit(view=None)
                    except Exception:
                        pass # Ignore errors if editing fails
                    break
            ...
            await channel.send(embed=create_embed("Error", "I lack permissions to move this channel to the archive or edit its permissions.", discord.Color.red()))
        except Exception as e:
...
async def set_prefix(ctx: commands.Context, prefix: str):
    if len(prefix) > 5:
         await send_embed_response(ctx, "Error", "Prefix cannot be longer than 5 characters.", discord.Color.red())
         return
...
def is_staff():
    async def predicate(ctx: commands.Context) -> bool:
        ...
        # Check if author is a member (might fail in weird scenarios)
        if not isinstance(ctx.author, discord.Member):
            return False
        if (staff_role and staff_role in ctx.author.roles) or ctx.author.guild_permissions.administrator:
            return True
        else:
...
async def remove(ctx: commands.Context, user: discord.Member):
    ...
    await ctx.channel.set_permissions(user, overwrite=None) # Reset permissions for the user
...
async def rename(ctx: commands.Context, *, new_name: str):
    ...
    try:
        ...
        await send_embed_response(ctx, "Channel Renamed", f"Channel name changed to `{clean_name}` by {ctx.author.mention}.", discord.Color.blue(), ephemeral=False)
    except Exception as e:
...
async def claim(ctx: commands.Context):
    ...
    if ctx.channel.topic and "claimed-by" in ctx.channel.topic:
        ...
        await send_embed_response(ctx, "Error", f"This ticket is already claimed by {claimer}.", discord.Color.orange())
        return

    base_topic = (ctx.channel.topic or "").split(" ")[0] # Get the 'ticket-user-ID' part
    if not base_topic.startswith("ticket-user-"): # Ensure topic format is correct
         base_topic = f"ticket-user-{ctx.channel.id}" # Fallback if topic is missing/wrong
...
async def unclaim(ctx: commands.Context):
    ...
    if not base_topic.startswith("ticket-user-"): # Ensure topic format is correct
         base_topic = f"ticket-user-{ctx.channel.id}" # Fallback
...
async def blacklist(ctx: commands.Context, user: discord.Member, *, reason: str):
    ...
    if user.id == ctx.author.id:
        await send_embed_response(ctx, "Error", "You cannot blacklist yourself.", discord.Color.orange())
        return
    if user.bot:
        await send_embed_response(ctx, "Error", "You cannot blacklist bots.", discord.Color.orange())
        return
...
async def ticket_stats(interaction: discord.Interaction):
    ...
    if ticket_category_id:
        ...
        if ticket_category and isinstance(ticket_category, discord.CategoryChannel):
            ...
            await interaction.followup.send(embed=create_embed("Warning", "Ticket category not found or invalid. Open count may be inaccurate.", discord.Color.orange()))
...
try:
    ...
    print("Login Failure: Improper token passed. Make sure your DISCORD_TOKEN in .env is correct.")
except discord.errors.PrivilegedIntentsRequired:
    print("Privileged Intents Required: Make sure you have enabled Presence, Server Members, and Message Content intents in the Discord Developer Portal.")
except Exception as e:
